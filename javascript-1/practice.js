...
const colorCopy = faveColors.slice(faveColors)
...
greeter should be a method that returns the string 
...
const filtered = myNumbers.filter(function(val){
	return val > 100
})
...
const total = myNumbers.reduce(function(val, sum){
	return val + sum;
})
...
const forTheLoveOfGeorge = notGeorge.map(function(string){
	string = 'George'
	return string
})
...
const enemies = people.filter(function(enemy){
...
const totallyAwesome = people.reduce(function(awesomeness, sum){
	return awesomeness.people += sum
})
