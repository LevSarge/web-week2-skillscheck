...
console.log('File connected, sarge')
...
Inside of addItem, invoke displayData() which is a function you did not create (we did) that takes care of some of the work for getting your page to display correctly.
*/
const addItem = (item) => {
    item = groceries.push(item)
    displayData()
}
...
//Inside of the removeItem function, remove the item that corresponds to the index 
//parameter from the groceries array above by using splice. 

//Then, invoke the displayData() function inside of your removeItem function.

const removeItem = (index) =>{
    index = groceries.splice(index)
    displayData()
}
...
const checkGroceryCount = () =>{
    if(groceries.length >=5){
        ...
        return '1 item'
    }else return `${groceries.length} items`
}
